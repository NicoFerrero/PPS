...
export class AuthService {
  ...
  enviarMensaje(aula: string, mensaje: Mensaje) {
    return this.afs.collection(`Chat-${aula}`).add({
      fecha: mensaje.fecha,
  ...
  traerMensajes(aula) {
    return this.afs.collection(`Chat-${aula}`, (ref) => ref.orderBy('fecha', 'asc')).valueChanges();
  }
}
